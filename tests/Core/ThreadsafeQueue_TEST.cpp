...
protected:
    ...
    {
        TestedOperation<ThreadsafeQueue<T>, void>::object().push(values_to_insert[i]);
    }
...
public:
    QueuePusher(ObjectWrapper<ThreadsafeQueue<T>>& wrapper,
        ...
        TestedOperation<ThreadsafeQueue<T>,void>{wrapper,ready},
        values_to_insert{},
        pushes_nmbr_{0}
    {}
    virtual ~QueuePusher() {}
...
// private:
//     AsyncTestingBase<ThreadsafeQueue<T>,void>& base_;
// protected:
...
//     }
// public:
//     QueueTryPopPtr(AsyncTestingBase<ThreadsafeQueue<T>,void>& base):
//         PObject{base_},
//         Enabler{base_},
//         base_{base}
//     {}
...
protected:
    ...
    void addValues(std::vector<T> values)
    {
        ...
        pusher_.setInvocationsCount(values.size());
    }
};

// template<typename T>
// class ThreadsafeQueueTest2:
//      public testing::Test
// {
// protected:
